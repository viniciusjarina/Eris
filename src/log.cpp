...
#ifdef __WINS__
_LIT(KLogFileName, "c:\\debug.txt");
#else
_LIT(KLogFileName, "c:\\DATA\\debug.txt");
#endif
_LIT(KPanic, "LogFile");
...
void LOG(TRefByValue<const TDesC> aFormat, ...)
{
    ...
    {
        PanicIfError(file.Create(fs, KLogFileName,EFileWrite | EFileShareExclusive));
    }
    else
    {
